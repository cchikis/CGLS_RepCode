%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Filename: kappa36_param.m
% Author: Craig A. Chikis
% Date: 01/21/2020
% Note(s): 
% MA-MFA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function m = kappa36_param()
	...
	markup_eq = nan; 
	markup_eq_array = nan(1,s_max+1); 
	markup_wt = nan;
	markup_wt_array = nan(1, s_max+1);
	profit = nan(1,2*s_max+1); 
	nu_s = nan(1,s_max+1);
	...
	end_hjb_error = nan;
	vfi_error_rho = nan;
	vfi_hjb_error_rho = nan;
	end_hjb_error_rho = nan;

	lab_demand = cell(1,3);
	lab_demand{1} = nan(1,2*s_max+1);
	lab_demand{2} = nan(1,2*s_max+1);
	lab_demand{3} = nan(1,s_max+1);
