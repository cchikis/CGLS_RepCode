...
% MA-MFA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function v_out = vfi_main(value_function_in, investment_in, eta, ...
																beta, s_max, ...
																wage_share, B, gamma, xbar, entrance, ...
																subsidy, lambda, rho, xE, ...
																B_entrant, probL, probF, prob0, ...
																probE, prob_exog, profit, ...
																EIS, r_guess)
	...
	profit_d = 1./(rho_term + xbar_term + eta);

	profitL = (profit((s_max+2):end) - (1 - subsidyL).*G(xL)*wage_share).*profit_d(2:end);
	profitF = (flip(profit(1:s_max)) - (1 - subsidyF).*G(xF)*wage_share).*profit_d(2:end);
	profit0 = (profit(s_max+1) - (1 - subsidy0)*G(x0)*wage_share).*profit_d(1);
