...
while (~clear_labor)
	...
	if (count_labor == 1)
		...
		if (entrance) && (strcmp(entry_type, "Undirected"))
	...
	while (~clear_value)
		...
		xL = max(GPInv(FOC_L./((1-subsidy((s_max+2):end))*w)), 0);
		xF = max(GPInv(FOC_F./((1-flip(subsidy(1:s_max)))*w)), 0);
		x0 = max(GPInv(FOC_0/((1-subsidy(s_max+1))*w)), 0);
		...
		wage_share = w;
		v_guess = vfi_alt(value_function_in, investment_in, eta, ...
																beta, s_max, ...
	...
	if (abs(labor_diff) < lab_tol) || (PE) || ((w > .9999) && (count_labor > 100)) || ...
		((w < 1e-4) && (count_labor > 100)) 
		clear_labor = true;
		labor_flag = true;

		value_functions = v_guess;
